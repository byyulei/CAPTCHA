...
class LoadImageContent:
    """YOLOv5 image/video dataloader, i.e. `python detect.py --source image.jpg/vid.mp4`"""

    def __init__(self, img, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        self.img = img
        self.img_size = img_size
        self.stride = stride
        self.nf = 1
        self.video_flag = [False] * 1
        self.mode = "image"
        self.auto = auto
        self.vid_stride = vid_stride  # video frame-rate stride
        self.cap = None
    ...
    def __next__(self):
        ...
        if isinstance(self.img,str):
            im0 = cv2.imread(self.img)  # BGR
        elif isinstance(self.img,bytes):
            img1 = np.frombuffer(self.img, np.uint8)
            im0 = cv2.imdecode(img1, cv2.IMREAD_ANYCOLOR)
        else:
            im0 = self.img

        s = f"image {self.count}/{self.nf} {'【图片路径】'}: "
