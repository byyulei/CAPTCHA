...
class YOLOv8_seg:
    ...
    def __init__(self, onnx_model, imgsz=(320, 320), infer_tool='openvino'):
        """
        Initialization.
    ...
    def __call__(self, file, conf_threshold=0.4, iou_threshold=0.45, nm=32):
        """
        ...
        if isinstance(file, str):
            im0 = cv2.imread(file)  # BGR
        elif isinstance(file, bytes):
            img1 = np.frombuffer(file, np.uint8)
            im0 = cv2.imdecode(img1, cv2.IMREAD_ANYCOLOR)
        else:
            im0 = file
        # 前处理Pre-process
        ...
        if self.infer_tool == 'openvino':
            ...
            preds = self.ort_session.run(None, {self.ort_session.get_inputs()[
                                                    0].name: im})  # 与bbox区别，输出是个列表，[检测头的输出(1, 116, 8400), 分割头的输出(1, 32, 160, 160)]
        ...
        # print('数据处理时间：{:.3f}s'.format(time.time() - t3))
    ...
    def draw_and_visualize(self, file, bboxes, segments, vis=False, save=True):
        if isinstance(file, str):
            im = cv2.imread(file)  # BGR
        elif isinstance(file, bytes):
            img1 = np.frombuffer(file, np.uint8)
            im = cv2.imdecode(img1, cv2.IMREAD_ANYCOLOR)
        else:
            im = file
        ...
        """
        # 创建一个同尺寸的图片
        copy_img = np.zeros(im.shape, dtype=np.uint8)


        # Draw rectangles
        for (*box, conf, cls_), segment in zip(bboxes, segments):
            cv2.polylines(im, np.int32([segment]), True, (255, 255, 255), 2)  # white borderline
            cv2.fillPoly(im, np.int32([segment]), (255, 0, 0))

            # 获取目标点
            cv2.fillPoly(copy_img, np.int32([segment]), (255, 0, 0))

            # 应用中值滤波，去除曲线毛刺
            copy_img = cv2.medianBlur(copy_img, 7)
            skeleton = skeletonize(copy_img)

            # cv2.imwrite('copy_img.png',copy_img)
...
# parser.add_argument('--model', type=str, default='best.onnx', help='Path to ONNX model')
parser.add_argument('--model', type=str, default='best_new.onnx', help='Path to ONNX model')
parser.add_argument('--imgsz', type=tuple, default=(320, 320), help='Image input size')
